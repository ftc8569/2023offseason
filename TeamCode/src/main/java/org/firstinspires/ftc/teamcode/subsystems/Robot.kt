...
enum class ArmState {
    SCORE_GROUND, SCORE_LOW, SCORE_MEDIUM, SCORE_HIGH, INTAKE, TRAVEL, CONE_RIGHTING, START
}
...
class Robot(
    ...
    private val isTelemetryEnabled: Boolean = false
) : com.arcrobotics.ftclib.command.Robot() {
    ...
    val turret: TurretSubsystem =
        TurretSubsystem(this, MotorEx(hardwareMap, "turret"), homingResult)
    val elbow = ElbowSubsystem(
        this,
        MotorEx(hardwareMap, "leftElbow"),
        MotorEx(hardwareMap, "rightElbow"),
        homingResult
    )
    ...
    private val rightDifferentialServo =
        AxonServo(hardwareMap, "rightWrist", 500.0, 2500.0, 355.0, true)
    val wrist: DifferentialWristSubsystem =
        DifferentialWristSubsystem(this, leftDifferentialServo, rightDifferentialServo)
    ...
    private fun homeMecahnisms(
        limitSwitches: LimitSwitchesSubsystem,
        leds: LEDSubsystem,
        telemetry: Telemetry
    ): HomingResult {
...
data class HomingResult(
    val homeAngles: LimitSwitchHomeAngles,
    val homePosition: HomePosition,
    val method: HomingMethod
)